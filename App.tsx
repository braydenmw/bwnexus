...
import type { SymbiosisContext, ReportParameters, ChatMessage, DashboardIntelligenceItem } from './types';
...
export type View = 'profile' | 'dashboard' | 'report' | 'compliance';
...
const App: React.FC = () => {
  ...
  // Dashboard State
  const [currentRegion, setCurrentRegion] = useState<string>(GLOBAL_REGIONS[0]);
  const [analysisItem, setAnalysisItem] = useState<DashboardIntelligenceItem | null>(null);
  ...
  const handleGenerateReport = useCallback(async (params: ReportParameters) => {
    ...
    // No view change needed here, we stay in 'report' view
  ...
  const handleResetReport = () => {
    ...
    setReportContent('');
    setCurrentView('report'); // Stay on the report page to show the generator
  };
  ...
  const handleAnalyzeDashboardItem = useCallback((item: DashboardIntelligenceItem) => {
  ...
  const renderView = () => {
      switch (currentView) {
          ...
          case 'dashboard':
              return <Dashboard 
                        onAnalyze={handleAnalyzeDashboardItem}
