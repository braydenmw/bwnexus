...
import type { DashboardIntelligenceItem } from '../types';
import Spinner from './Spinner';
import { CloseIcon, DownloadIcon, NexusLogo } from './Icons';

interface AnalysisModalProps {
  item: DashboardIntelligenceItem;
  region: string;
  onClose: () => void;
}
...
export const AnalysisModal: React.FC<AnalysisModalProps> = ({ item, region, onClose }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
        ...
        pdf.save(`Nexus-Analysis-${item.company.replace(/\s+/g, '-')}.pdf`);
    } catch (e) {
        ...
        <header className="p-4 flex justify-between items-center border-b border-nexus-border flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 truncate max-w-md">Topic: {item.company} | Region: {region}</p>
            </div>
          </div>
            ...
            <div id="analysis-report-content" className="p-5">
              {isLoading && !report && (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 min-h-[300px]">
                  <Spinner />
                  <p className="mt-2">Generating Analysis for {item.company} in {region}...</p>
                </div>
              )}
              {error && (
